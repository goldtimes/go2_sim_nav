...
def merge_bags(input_bags, output_bag):
    ...
    for input_bag in input_bags:
        ...
        logging.info('converting %s', input_bag)
...
def convert_ros1_bag(input_file, output_file=None):
    ...
    try:
        # 读取压缩文件并解压
        logging.info('rosbags-convert --src ' + input_file + ' --dst ' + output_file)
        os.system('rosbags-convert --src ' + input_file + ' --dst ' + output_file)

        logging.info(f"转换成功: {input_file} -> {output_file}")
        return output_file
...
def decompress_db3_zstd(input_file, output_file=None):
    ...
    try:
        ...
        logging.info(f"解压成功: {input_file} -> {output_file}")
        return output_file
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 2:
        logging.error('Usage: python merge_bags.py path_to_your_dir')
        exit(1)
    ...
    logging.info('找到了 %d 个数据包', len(bags))
    ...
    for bag in bags:
        ...
        logging.info(bag["path"].rsplit('.', 1)[0])
        db3_name = bag["path"].rsplit('.', 1)[0] + '/' + filename + '.db3'

        logging.info('db3 should be %s', db3_name)
