...
void TiledMap::SaveToBin(bool only_dynamic) {
    LLOG_INFO(logging::kMap, "map is saved to {}, sz: {}", options_.map_path_, static_chunks_.size());
...
bool TiledMap::LoadMapIndex() {
    ...
    if (!fin) {
        LLOG_ERROR(logging::kMap, "cannot load map index from: {}", options_.map_path_);
        return false;
    }

    LLOG_INFO(logging::kMap, "loading maps");
    ClearMap();
        ...
        if (reading_fp) {
            ...
            if (options_.policy_ == DynamicCloudPolicy::PERSISTENT) {
                ...
                if (PathExists(dyn_filename)) {
                    LLOG_DEBUG(logging::kMap, "load dynamic chunk: {}", dyn_filename);
    ...
    LLOG_INFO(logging::kMap, "loaded chunks: {}, fps: {}", static_chunks_.size(), func_points_.size());

    fin.close();

    /// 索引解析出 0 个区块 = 地图实际不可用（index.txt 为空/格式错）。
    /// 必须当失败上报：否则上层以为"加载成功"，拿着空地图一路跑到 GICP 匹配才发现，
    /// 报错点离根因很远，极难排查。
    if (static_chunks_.empty()) {
        LLOG_ERROR(logging::kMap, "no map chunk parsed from index: {} (empty or malformed index.txt)",
                   options_.map_path_);
        return false;
    }

    /// 统计点云读取失败的区块数（LoadCloud 内部已逐个打 ERROR，这里给个汇总）
    size_t failed_chunks = 0;
    for (const auto& kv : static_chunks_) {
        if (!kv.second->loaded_) {
            failed_chunks++;
        }
    }
    if (failed_chunks > 0) {
        LLOG_ERROR(logging::kMap, "{} of {} map chunks failed to load from {}", failed_chunks, static_chunks_.size(),
                   options_.map_path_);
        if (failed_chunks == static_chunks_.size()) {
            LLOG_ERROR(logging::kMap, "all map chunks unusable, treat as load failure");
            return false;
        }
    }
    ...
    if (!fin) {
        ...
        LLOG_ERROR(logging::kMap, "找不到动态区域文件，不使用动态区域多边形");
        return true;
    }
    ...
    LLOG_INFO(logging::kMap, "dynamic polygons: {}", dynamic_polygon_.size());
...
void TiledMap::ResetDynamicCloud() {
    ...
    if (id_to_grid_.empty()) {
        LLOG_WARN(logging::kMap, "重置动态图层时，原始地图索引为空");
        return;
    }
    ...
    if (options_.policy_ == DynamicCloudPolicy::PERSISTENT) {
        ...
        for (auto it = id_to_grid_.begin(); it != id_to_grid_.end(); it++) {
            ...
            std::string dyn_filename = options_.map_path_ + "/" + std::to_string(id) + "_dyn.pcd";
            if (PathExists(dyn_filename)) {
                LLOG_DEBUG(logging::kMap, "reset dynamic chunk: {}", dyn_filename);
                auto dyn_chunk = std::make_shared<MapChunk>(id, grid, dyn_filename);
                dynamic_chunks_.emplace(grid, dyn_chunk);
        ...
        if (init_dy_size > 0) {
            LLOG_INFO(logging::kMap, "reset dynamic map success, load size: {}", init_dy_size);
        }
    }
