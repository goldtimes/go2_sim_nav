...
void NullLocalPlanner::setSpeedLimit(double v_limit)
{
  speed_limit_ = v_limit;
}

void NullLocalPlanner::setCostMap(std::shared_ptr<const CostMap2D> local_inflated)
{
...
void NullLocalPlanner::setDynamicObstacles(const std::vector<DynamicObstacle> &obs)
{
  dynamic_obs_ = obs;
}

LocalPlanResult NullLocalPlanner::computeCommand(const Pose2D &pose, double dt)
{
  (void)pose;
  (void)dt;
  ++command_calls_;
