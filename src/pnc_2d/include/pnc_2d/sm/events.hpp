...
//
// 事件是"外面发生了什么事"，**不带动作**：动作由转移表决定（见 manager_sm.hpp 的
// SideEffect）。这样"什么时候做什么"全部集中在表里，节点只负责把 ROS 世界
...
//   kStuck         ← 节点自己检测（跟随中长时间没有前进，见 manager 的 stuck 判据）
//   kOdomJump      ← 定位跳变（与上一次位姿偏差过大，缓存的位姿已不可信）
//   kRecoveryDone/Fail ← 恢复行为执行结果（P6 才有实现）
//   kCancel        ← 显式取消服务 / 新目标顶掉旧目标
