...
#include "core/common/miao_log.h"
#include <execution>
...
void Optimizer::InitFromRaw(int level) {
    ...
    if (edges_.empty()) {
        MLOG_WARN("Attempt to initialize an empty graph");
        return;
    }
...
void Optimizer::InitFromLast(int level) {
    ...
    for (const auto &v : vertices_) {
        if (v.second->Marginalized()) {
            MLOG_ERROR("should not use marginalization in incremental mode. ");
            return;
        }
    }
...
int Optimizer::Optimize(int iterations) {
    if (iv_map_.empty()) {
        MLOG_WARN("0 vertices to Optimize, maybe forgot to call InitializeOptimization()");
    ...
    ok = algorithm_->Init();
    if (!ok) {
        MLOG_ERROR("Error while initializing");
        return -1;
    }
    ...
    for (int i = 0; i < iterations && !Terminate() && ok; i++) {
        ...
        if (Verbose()) {
            MLOG_INFO("iteration= {}\t chi2= {:.6f}\t edges= {}", i, this_chi2, active_edges_.size());
        }
...
bool Optimizer::AddVertex(std::shared_ptr<Vertex> v) {
    if (config_.incremental_mode_) {
        if (config_.max_vertex_size_ > 0 && vertices_.size() == config_.max_vertex_size_) {
            ...
            if (verbose_) {
                MLOG_INFO("replacing new vertex {} with old {}", v->GetId(), v_remove->GetId());
            }
