...
Params::Params(const std::string& yaml_path) {
    ...
    LLOG_INFO(logging::kCommon, "Loading params from {}", yaml_path);
    ...
    try {
        ...
        LLOG_ERROR(logging::kCommon, "YAML parsing error at line {}, column {}: {}", e.mark.line + 1,
                   e.mark.column + 1, e.what());
        // 退出程序
