...
def generate_launch_description():
    ...
    # 仿真里是否启动 robot_localization 的 EKF（默认关）。
    # 用 lightning 做里程计/定位时不需要它；而且它会与 QuadrupedOdometryNode
    # (enable_odom_tf: True) 重复发布 odom->base_link，同一个 TF 对有两个源。
    # 需要恢复时： ros2 launch gazebo_sim launch.py ... enable_ekf:=true
    enable_ekf = LaunchConfiguration('enable_ekf', default='false')
    declare_enable_ekf = DeclareLaunchArgument(
        name='enable_ekf', default_value='false',
    ...
    for i, robot in enumerate(robots):
        ...
        spawn_entity = Node(
            ...
            arguments=[
                ...
                # 禁止重命名：世界里若已存在同名 model 就直接报错，而不是静默生成
                # robot1_my_bot_0/_1...（那些狗的话题名不变，会往同一批话题重复发数据）。
                # 改配置后请连 world 一起重启，别只重启本 launch。
        ...
        # robot_localization EKF：默认不开（由 enable_ekf 控制）
