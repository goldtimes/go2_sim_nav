...
bool PGOImpl::Reset() {
    LLOG_WARN(logging::kPgo, "PGO is reset");
    CleanProblem();
    frames_.clear();
    frames_by_id_.clear();
...
void PGOImpl::AddPGOFrame(std::shared_ptr<PGOFrame> pgo_frame) {
    ...
    if (last_frame_ != nullptr) {
        ...
        if (adjacent_dalta_t < 0.) {
            LLOG_WARN_THROTTLE(logging::kPgo, 2000, "PGO received pgoframe, however timestamp rollback for {}senonds!",
                               adjacent_dalta_t);
            return;
        }
    ...
    if (debug_) {
        LLOG_DEBUG(logging::kPgo, "inserting pgo frame, it's timestamp is {:.18f}", pgo_frame->timestamp_);
    }
    ...
    if (!interp_lio_success && !interp_dr_success) {
        LLOG_ERROR(logging::kPgo, "PGO received pgo frame, but assign relative pose failed!");
        return;
    }

    is_in_map_ = pgo_frame->lidar_loc_set_ && pgo_frame->lidar_loc_valid_;
    if (!is_in_map_) {
        LLOG_ERROR_THROTTLE(logging::kPgo, 2000, "PGO received PGOFrame with lidar_loc_set_({}), lidar_loc_valid_({}); Reject It!",
                            pgo_frame->lidar_loc_set_, pgo_frame->lidar_loc_valid_);
        return;
    }
...
bool PGOImpl::AssignLidarOdomPoseIfNeeded(std::shared_ptr<PGOFrame> frame) {
    ...
    if (!frame->lidar_odom_set_) {
        if (lo_interp_done) {
            ...
            LLOG_WARN_THROTTLE(logging::kPgo, 2000, "PGOFrame (frame_id {}) Interpolate on lidarOdom Failed! time: {:.18f}, "
                                           "latest lidarOdom time: {:.18f}",
                               frame->frame_id_, frame->timestamp_,
                               lidar_odom_pose_queue_.empty() ? 0.0 : lidar_odom_pose_queue_.back().timestamp_);
            return false;
        }
    } else {
        return true;
...
bool PGOImpl::AssignDRPoseIfNeeded(std::shared_ptr<PGOFrame> frame) {
    ...
    if (!frame->dr_valid_) {
        if (dr_interp_done) {
            ...
            // DR理论上应该比 lidarloc 快。
            LLOG_WARN_THROTTLE(logging::kPgo, 2000, "PGOFrame (frame_id{}) Interpolate on DR Failed!", frame->frame_id_);
            return false;
        }
    } else {
        return true;
...
void PGOImpl::RunOptimization() {
    ...
    if (debug_) {
        LLOG_DEBUG(logging::kPgo, "Run pose graph optimization: ");
    }
...
void PGOImpl::RemoveOutliers() {
    ...
    auto remove_outlier = [&cnt_outliers, this](miao::Edge* e) {
        ...
        if (e->Chi2() > e->GetRobustKernel()->Delta()) {
            ...
            if (debug_) {
                LLOG_DEBUG(logging::kPgo, "frame {} has outlier gps, chi2 = {} > {}", frame->frame_id_, e->Chi2(),
                           e->GetRobustKernel()->Delta());
            }
        } else {
            auto frame = frames_by_id_.find(e->GetVertex(0)->GetId())->second;
            if (debug_) {
                LLOG_DEBUG(logging::kPgo, "frame {} has inlier gps, chi2 = {} < {}", frame->frame_id_, e->Chi2(),
                           e->GetRobustKernel()->Delta());
            }
...
void PGOImpl::CollectOptimizationStatistics() {
    /// 打印必要信息
    if (debug_) {
        LLOG_DEBUG(logging::kPgo, "LidarLoc       -- {}", print_info(lidar_loc_edges_, options_.lidar_loc_outlier_th));
        LLOG_DEBUG(logging::kPgo, "LidarOdom      -- {}", print_info(lidar_odom_edges_));
        LLOG_DEBUG(logging::kPgo, "DR             -- {}", print_info(dr_edges_));
        LLOG_DEBUG(logging::kPgo, "Marginal Prior -- {}", print_info(prior_edges_));
    }
