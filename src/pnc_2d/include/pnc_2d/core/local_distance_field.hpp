...
//     把稀疏采样"糊"成连续场。
//   · **图外 = 很远**（同样是乐观策略，与 ClearanceField 的 out-of-map
//   处理一致；
...
public:
  ...
  /// @param fill_radius    邻域填充半径 [格]；0 = 不填充（只适合采样间距 ≈
  /// 分辨率）
  ...
  ///   如果把它当成后者（setDistanceField(nullptr)），MPC 会进入降级并限速到
  ///   0.3 m/s —— 车会在空旷大路上爬行，而原因看起来像"感知挂了"，极难排查。
  ...
  /// 把**禁行区**并进距离场（每帧重建后都要调，见 local_planner_node）。
  ///
  /// 为什么必须并进来：局部用的是感知的滑动窗 + ESDF，**里面没有任何区域**
  /// （感知不认识区域语义）。不并的话车可以贴着禁行区开过去 ——
  /// 今天只有"订阅全局图"
  /// 的模块才遵守禁行区（全局规划器/RViz），**局部是盲的**。
  ///
  /// 语义：目标值 = `max(0, 点到区域多边形的距离 − inflate)`（区内 = 0），
  /// 逐格取 `min(现有距离, 目标值)`；inflate 与 map_server
  /// 烧全局图用的是**同一个值** （由 ZoneArray
  /// 消息带过来），所以两边对"能不能过"的判定一致。
  ///
  /// @param band 只更新"距区域包围盒 ≤ band"的格子 [m]；不需要扫全图（见 .cpp
  /// 注释）
  /// @return 被压低的格数（0 = 没有禁行区，场一字未改）
  ...
  std::size_t filledCells() const {
    return filled_cells_;
  } ///< 靠邻域填充补出来的格数
  std::size_t maxFillUsed() const {
    return max_fill_used_;
  } ///< 实际用到的最大填充半径
