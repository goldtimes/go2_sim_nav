...
void SignalHandler(int signum) {
    if (slam_instance) {
        LLOG_INFO(lightning::logging::kApp, "SIGINT received, saving map and path...");
        slam_instance->SaveMap();
        slam_instance->SavePath();
    }
    rclcpp::shutdown();
    LLOG_INFO(lightning::logging::kApp, "Shutdown complete.");
...
int main(int argc, char** argv) {
    ...
    if (!slam.Init(FLAGS_config)) {
        LLOG_ERROR(lightning::logging::kApp, "failed to init slam");
        lightning::logging::Shutdown();
        return -1;
    }
