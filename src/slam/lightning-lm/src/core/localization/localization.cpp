...
bool Localization::Init(const std::string& yaml_path, const std::string& global_map_path) {
    ...
    if (lidar_loc_ != nullptr) {
        // 若已经启动，则变为重新初始化。
        // 注意传 save_dyn=false：这条路径只在运行时换图时走到，
        // 旧地图的动态图层不应在“切换”时落盘（那是正常退出才做的事）。
        Finish(/*save_dyn=*/false);
    }
    ...
    if (!lio_->Init(yaml_path)) {
        LLOG_ERROR(logging::kLoc, "failed to init lio");
        return false;
    }
    ...
    /// 地图加载失败则整个定位初始化失败。
    /// 注：此处旧的 lidar_loc_ 已在函数开头 Finish() 掉，所以失败后本次会话不可用；
    /// 正因如此，换图路径必须在调用本函数之前做完整的目录预检（见 LocSystem::LoadMap）。
    if (!lidar_loc_->Init(yaml_path)) {
        LLOG_ERROR(logging::kLoc, "failed to init lidar loc (map not usable)");
        lidar_loc_.reset();
        return false;
    }
    ...
    LLOG_INFO(logging::kLoc, "lidar_type {}", lidar_type);
    if (lidar_type == 1) {
        preprocess_->SetLidarType(LidarType::AVIA);
        LLOG_INFO(logging::kLoc, "Using AVIA Lidar");
    } else if (lidar_type == 2) {
        preprocess_->SetLidarType(LidarType::VELO32);
        LLOG_INFO(logging::kLoc, "Using Velodyne 32 Lidar");
    } else if (lidar_type == 3) {
        preprocess_->SetLidarType(LidarType::OUST64);
        LLOG_INFO(logging::kLoc, "Using OUST 64 Lidar");
    } else if (lidar_type == 4) {
        preprocess_->SetLidarType(LidarType::ROBOSENSE);
        LLOG_INFO(logging::kLoc, "Using Robosense Lidar");
    } else if (lidar_type == 5) {
        preprocess_->SetLidarType(LidarType::SIMULATED);
        LLOG_INFO(logging::kLoc, "Using Simulated Lidar (instantaneous scan, no per-point time)");
    } else {
        LLOG_ERROR(logging::kLoc, "unknown lidar_type");
    }
...
void Localization::LidarOdomProcCloud(CloudPtr cloud) {
    ...
    LLOG_DEBUG(logging::kLoc, "proj cloud size: {}", scan->size());
...
void Localization::LidarLocProcCloud(CloudPtr scan_undist) {
    ...
    if (loc_state_callback_) {
        ...
        LLOG_DEBUG(logging::kLoc, "loc_state: {}", loc_state->data);
...
void Localization::ProcessIMUMsg(IMUPtr imu) {
    ...
    if (last_imu_time_ > 0 && this_imu_time < last_imu_time_) {
        LLOG_WARN_THROTTLE(logging::kLoc, 2000, "IMU 时间异常：{:.12f}, last: {:.12f}", this_imu_time, last_imu_time_);
    }
...
void Localization::Finish(bool save_dyn) {
    /// 地图加载失败时 lidar_loc_ 可能已被 reset，必须判空
    if (lidar_loc_) {
        lidar_loc_->Finish(save_dyn);
    }
    if (ui_) {
        ui_->Quit();
    }
...
void Localization::SetExternalPose(const Eigen::Quaterniond& q, const Eigen::Vector3d& t) {
    UL lock(global_mutex_);
    /// 设置外部重定位的 pose（语义：地图系 IMU 位姿），作为 LidarLoc 的初始化种子。
    /// 经典 odom 模型：LO(ESKF) 保持自身 odom 系连续积分，**不再** SetInitPose 搬到地图绝对坐标，
    /// 否则 odom 原点被拉到地图原点（map→odom≈I），失去“odom 原点 = 机器人启动/重定位位置”的语义。
    /// odom 与 map 的衔接由 LocSystem::HandleLocTf 用 map→odom = map→base × (odom→base)⁻¹ 合成；
    /// LidarLoc 的 GICP 初值只用 LO 的相对增量（last_abs_pose_ × δ_LO），与 LO 绝对参考系无关。
    if (lidar_loc_) {
