...
struct MappingParameters {
  ...
  bool pub_2d_occupancy_inflate_;  // 发膨胀后占据 OccupancyGrid（3D 双圆柱膨胀的 2D 投影）
  ...
  double proj_z_min_, proj_z_max_; // 投影高度带（map 系绝对高度 / 相对传感器高度）
  bool esdf_unknown_as_occupied_;  // 未知区域是否当障碍（默认 false：乐观，与 3D 层一致）
...
struct MappingData {
  ...
  /* ---------- 2D 局部层 ----------
   * x-y 与 3D 栅格同分辨率、同窗口（覆盖 [map_bound_min_idx_, map_bound_max_idx_]），
   * 但**按全局索引用连续下标排列**（i = ix + iy * nx），不做环形 wrap
   * —— 这样它就是一个世界系下真正的矩形，EDT 不会跨边界泄漏。
   * 3D 缓冲是环形下标的，所以构建时是“读 3D（自动 wrap）→ 写 2D（连续）”。
   */
  ...
  double proj_z_lo_, proj_z_hi_;      // 本帧实际使用的投影高度带（发布时写进 origin.z）
  /**
   * 构建时刻的窗口最小索引。
  ...
  std::vector<int> occ2d_occ_cnt_, occ2d_free_cnt_, occ2d_infl_cnt_;
...
public:
   ...
   * 并在 pub_2d_esdf_ 打开时计算 2D ESDF（到最近障碍的距离场）。
   *
   ...
   *   topic_2d_esdf_              sensor_msgs/PointCloud2  xyz + intensity(距离)
   * 带节流（pub_2d_interval_）与“无订阅者不发布”。
   */
  void publish2D();
...
inline float GridMap::getDistance2D(int x, int y) const {
  if (!md_.has_2d_ || md_.esdf2d_.empty())
    return -1.f;
  const int ix = x - md_.occ2d_min_idx_(0);
  const int iy = y - md_.occ2d_min_idx_(1);
  if (ix < 0 || iy < 0 || ix >= mp_.map_voxel_num_(0) ||
