...
class Localization {
   public:
    ...
    /// 结束，保存临时地图
    /// @param save_dyn 透传给 LidarLoc::Finish，运行时换图时应传 false
    ...
    /// 设置定位状态回调（供 rviz/Web 展示 IDLE/INITIALIZING/GOOD/FAIL 等）
    void SetLocStateCallback(const LocStateCallback& callback) { loc_state_callback_ = callback; }
