   ...
   public:
    ...
    void PushDRPose(const SE3& dr_pose) {
        ...
        if (!dr_queue_.empty()) {
            if ((dr_pose.translation() - dr_queue_.back().translation()).norm() >= smoother_dr_limit_trans_) {
                /// 无效
                LLOG_WARN_THROTTLE(logging::kPgo, 2000, "smoother motion is too large: {:.3f}",
    ...
    void PushPose(const SE3& pose) {
        ...
        if (dn > smoother_trans_limit_) {
            // 平滑器差别太大，直接跳过去并重置
            LLOG_WARN_THROTTLE(logging::kPgo, 2000, "smoother is too large, jump to target: {}, given: {}, output: {}",
                               (output_pose_.translation() - pose.translation()).transpose(),
                               pose.translation().transpose(), output_pose_.translation().transpose());
            output_pose_ = pose;
            pose_queue_.clear();
            return;
        } else if (dn > smoother_trans_limit2_) {
            // 快速收敛到正确位姿
            LLOG_DEBUG(logging::kPgo, "smoother factor set to 0.2");
            smooth_factor_ = 0.2;
        } else {
            // 回到默认值
            smooth_factor_ = 0.01;
