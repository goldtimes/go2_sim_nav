...
inline bool PoseInterp(double query_time, C&& data, FT&& take_time_func, FP&& take_pose_func, SE3& result,
    ...
    if (data.size() <= 1) {
        if (verbose) {
            LLOG_DEBUG(logging::kCommon, "data size is too small for interp: {}", data.size());
        }
        return false;
    }
    ...
    if (query_time > last_time) {
        if (verbose) {
            LLOG_WARN(logging::kCommon, "query time is later than last time.");
        }
        ...
        if (verbose) {
            LLOG_WARN(logging::kCommon, "pose interp failed: {}, {}, {}", query_time, (last_time + time_th),
        ...
        if (query_time < (first_time - time_th)) {
            if (verbose) {
                LLOG_WARN(logging::kCommon, "query time too early: {}, {}", query_time, (first_time - time_th));
            }
            return false;
        } else {
