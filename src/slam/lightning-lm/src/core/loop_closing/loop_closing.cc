...
void LoopClosing::Init(const std::string yaml_path) {
    ...
    if (options_.online_mode_) {
        LLOG_INFO(logging::kLoop, "loop closing module is running in online mode");
...
void LoopClosing::HandleKF(Keyframe::Ptr kf) {
    ...
    if (options_.verbose_) {
        LLOG_DEBUG(logging::kLoop, "lc: get kf {} candi: {}", cur_kf_->GetID(), candidates_.size());
    }
...
void LoopClosing::DetectLoopCandidates() {
    ...
    if (last_loop_kf_ && (cur_kf_->GetID() - last_loop_kf_->GetID()) <= options_.loop_kf_gap_) {
        LLOG_DEBUG(logging::kLoop, "skip because last loop kf: {}", last_loop_kf_->GetID());
        return;
    }
    ...
    if (options_.verbose_ && !candidates_.empty()) {
        LLOG_DEBUG(logging::kLoop, "lc candi: {}", candidates_.size());
    }
}
...
void LoopClosing::ComputeLoopCandidates() {
    ...
    if (options_.verbose_) {
        LLOG_DEBUG(logging::kLoop, "success: {}/{}", succ_candidates.size(), candidates_.size());
    }
...
void LoopClosing::PoseOptimization() {
    ...
    if (options_.verbose_) {
        LLOG_INFO(logging::kLoop, "loop outliers: {}/{}", cnt_outliers, edge_loops_.size());
    }
    ...
    LLOG_INFO(logging::kLoop, "optimize finished, loops: {}", edge_loops_.size());
}
